...
{
    public class MookSlash1 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            float len = (projectile.localAI[0] - 15) / 15f * 200f * projectile.scale;
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (bars.Count > 2)
            {
                ...
                // 把变换和所需信息丢给shader
                Furioso.MookEffect.Parameters["uTransform"].SetValue(model * projection);
                Furioso.MookEffect.Parameters["alpha"].SetValue(projectile.Opacity);
                Main.graphics.GraphicsDevice.Textures[0] = mod.GetTexture("Images/Extra_Mook");
                Main.graphics.GraphicsDevice.SamplerStates[0] = SamplerState.PointWrap;

                Furioso.MookEffect.CurrentTechnique.Passes[0].Apply();
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            if (!Main.player[projectile.owner].GetModPlayer<RolandPlayer>().FuriosoAttacking)
            {
                ...
                if (trueDamage < 20) trueDamage = 20;
                FuriosoUtils.StrikeNPC(target, trueDamage, 0, 0);
                Main.player[projectile.owner].addDPS(trueDamage);
