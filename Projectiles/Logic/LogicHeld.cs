...
namespace Furioso.Projectiles.Logic
{
    public class LogicHeld : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                Projectile.velocity = Vector2.Normalize(Main.MouseWorld - owner.Center) * 10;
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), owner.Center, Projectile.velocity, ModContent.ProjectileType<LogicHeld3>(), 0, 0, owner.whoAmI);
                float rot = Projectile.velocity.ToRotation();
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), owner.Center + new Vector2(0, -5) + rot.ToRotationVector2() * 150, rot.ToRotationVector2(), ModContent.ProjectileType<LogicGunExplosion2>(), (int)(Projectile.damage * 1.5f), Projectile.knockBack * 2f, owner.whoAmI);
