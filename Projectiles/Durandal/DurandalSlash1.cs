...
namespace Furioso.Projectiles.Durandal
{
    public class DurandalSlash1 : ModProjectile
    {
        ...
        Effect DurandalEffect = ModContent.Request<Effect>("Furioso/Effects/Content/DurandalEffect").Value;
        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] < 15)
            {
                ...
                {
                    r = -MathHelper.Pi / 2 + MathHelper.Pi / 8 * 11 * t;
                }
                else
                {
                    r = -MathHelper.Pi / 2 - MathHelper.Pi / 8 * 11 * t;
            ...
            if (Projectile.localAI[0] <= 15)
            {
                ...
                {
                    OldRot[i] = r - (float)i / OldRot.Length * MathHelper.Pi * 3f / 2f * Projectile.ai[0];
            ...
            else
            {
                ...
                for (int i = 0; i < OldRot.Length; i++)
                {
                    OldRot[i] = r - (float)i / OldRot.Length * MathHelper.Pi * 3f / 2f * Projectile.ai[0] * k;
                    OldPos[i] = owner.Center + OldRot[i].ToRotationVector2() * Radius;
                    OldRot[i] = OldRot[i] + MathHelper.Pi / 2 * Projectile.ai[0];
                }
            }
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            if (bars.Count > 2)
            {
                ...
                // 把变换和所需信息丢给shader
                DurandalEffect.Parameters["uTransform"].SetValue(model * projection);
                DurandalEffect.Parameters["alpha"].SetValue(Projectile.Opacity);
                ...
                DurandalEffect.CurrentTechnique.Passes[0].Apply();
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                Main.player[Projectile.owner].AddBuff(ModContent.BuffType<DurandalPower>(), 360);
                Main.player[Projectile.owner].GetModPlayer<RolandPlayer>().BlackSilenceCounter.AddCounter(BlackSilenceCounter.Attack.Durandal);
            }
            else
            {
